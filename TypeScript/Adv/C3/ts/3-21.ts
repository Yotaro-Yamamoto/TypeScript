...
/*このように、抽象クラスは、それを継承する具象クラスが必ず実装しなければならない性質
　(プロパティとメソッド)を定義する役割を果たす。誰がが具象クラスを作成しても、必ず
　分量をプロパティに持ち、分量を出力するメソッドを持つことを抽象クラスが保証する。
　ただし、抽象メソッドの処理内容については具象クラス側で自由に実装することができる。
　Drinkクラスが強制しているのはshowAmountというメソッドの名前、引数の有無と型、
　戻り値の型だけである。*/
